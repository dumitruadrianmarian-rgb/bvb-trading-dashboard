        ...
        recommendations.buy.forEach(rec => {
            ...
            card.innerHTML = `
                ...
                <div class="rec-card-news" id="rec-news-buy-${rec.symbol}">
                    <div class="loading-news" style="font-size: 0.8rem; color: var(--text-muted);"><i data-lucide="loader" class="spin"></i> Scanare flux de știri...</div>
                </div>
            `;
                        ...
                        newsEl.innerHTML = `
                            <div style="margin-top: 10px; padding: 4px 0 4px 12px; border-left: 2px solid ${sentimentColor};">
                                ...
                                <div style="font-size: 0.7rem; color: var(--text-muted); margin-top: 2px; display: flex; align-items: center; gap: 4px;">
                                    <i data-lucide="${sentimentIcon}" style="width:11px;height:11px;color:${sentimentColor}"></i>
...
function updateActiveTickerDetails(symbol) {
    ...
    document.getElementById("stat-sector").innerText = stock.sector || getStockSector(stock.symbol);
    
    // Technical Score
    const techScore = stock.technical.tech_score || 50.0;
...
function renderChart() {
    ...
    let alertsList = JSON.parse(localStorage.getItem("bvb_price_alerts") || "[]");
...
// Active Navigation Highlight on Scroll & Click
function setupScrollHighlight() {
    const navItems = document.querySelectorAll(".nav-item");
    const mobileNavItems = document.querySelectorAll(".mobile-nav-item");
    
    // Click listeners to change active tab instantly on click (desktop sidebar)
    ...
    // Click listeners to change active tab instantly on click (mobile bottom bar)
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                const id = entry.target.id || (entry.target.tagName === "HEADER" ? "dashboard" : "");
                let activeBtnId = "";
                if (id === "dashboard") activeBtnId = "btn-dashboard";
                else if (id === "recommendations") activeBtnId = "btn-recs";
                else if (id === "portfolio") activeBtnId = "btn-portfolio";
                else if (id === "chart-section") activeBtnId = "btn-analysis";
                else if (id === "watchlist") activeBtnId = "btn-watchlist";
                ...
                // Also update mobile navigation bar items
                mobileNavItems.forEach(item => {
    ...
    // Observe the main header (Dashboard) and other sections
    ...
    if (recsSection) observer.observe(recsSection);
    
    const portSection = document.getElementById("portfolio");
    if (portSection) observer.observe(portSection);
    
    const chartSection = document.getElementById("chart-section");
    if (chartSection) observer.observe(chartSection);
    
    const watchlistSection = document.getElementById("watchlist");
    if (watchlistSection) observer.observe(watchlistSection);
}
            ...
            combined.forEach(stock => {
                ...
                item.innerHTML = `
                    <div style="display: flex; flex-direction: column; align-items: flex-start; gap: 2px;">
                        ...
                        <span class="suggestion-name" style="max-width: 250px; text-align: left;" title="${stock.name}">${stock.name}</span>
                    </div>
                    ${indicator}
                `;
            ...
            if (isValidSymbolPattern && !alreadyExists) {
                ...
                addOption.innerHTML = `
                    <span class="suggestion-symbol">
                        <i data-lucide="plus-circle" style="width: 14px; height: 14px; margin-right: 6px; display: inline-block; vertical-align: middle;"></i>
                        Adaugă "${query}"
                    </span>
            ...
            if (localMatches.length > 0) {
                ...
                localMatches.forEach(stock => {
                    ...
                    item.innerHTML = `
                        <div style="display: flex; flex-direction: column; align-items: flex-start; gap: 2px;">
                            <span class="suggestion-symbol">${stock.symbol}</span>
                            <span class="suggestion-name" style="max-width: 250px; text-align: left;">${stock.name}</span>
                        </div>
                        <span class="suggestion-badge local">Watchlist</span>
                    `;
                    item.addEventListener("click", () => {
...
function showToast(message, type = 'success') {
    ...
    toast.innerHTML = `
        <i data-lucide="${iconName}" style="width: 18px; height: 18px; color: ${type === 'success' ? '#22c55e' : '#ef4444'}"></i>
        <span>${message}</span>
    `;
        ...
        tr.innerHTML = `
            ...
            <td>
                <div style="display: flex; align-items: center; gap: 4px;">
                    <input type="number" step="any" min="0" value="${item.dividends}" onchange="updatePortfolioDividends('${item.symbol}', this.value)" style="width: 70px; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1); color: #fff; padding: 2px 4px; border-radius: 4px; font-size: 12px; font-weight: 600; text-align: right;">
        ...
        if (enrichedItems.length === 0) {
            ...
            let adviceHtml = `
                <div style="font-weight: 700; font-size: 13px; color: var(--text-primary); margin-bottom: 8px; display: flex; align-items: center; gap: 6px;">
                    <i data-lucide="shield-alert" style="width:16px;height:16px;color:var(--color-blue);"></i>
...
function checkPriceAlerts() {
    ...
    let alerts = JSON.parse(localStorage.getItem("bvb_price_alerts") || "[]");
    ...
    alerts.forEach(alert => {
        ...
        if (isTriggered) {
            triggered = true;
            showToast(`🔔 ALERTĂ PREȚ: ${alert.symbol} a atins targetul de ${target.toFixed(2)} RON! (Preț curent: ${currentPrice.toFixed(2)} RON)`, "success");
            
            // Dacă browserul suportă notificări de sistem
    ...
    if (triggered) {
        localStorage.setItem("bvb_price_alerts", JSON.stringify(remainingAlerts));
        renderPriceAlertsList();
    }
}
...
function setPriceAlert() {
    ...
    // Verifică duplicat
    if (alerts.some(a => a.symbol === activeSymbol && a.target === target)) {
    ...
    localStorage.setItem("bvb_price_alerts", JSON.stringify(alerts));
    targetInput.value = "";
    
    renderPriceAlertsList();
    renderChart();
    showToast(`Alertă setată pentru ${activeSymbol} la ${target.toFixed(2)} RON.`, "success");
...
function renderPriceAlertsList() {
    ...
    listDiv.innerHTML = activeAlerts.map(alert => {
        ...
        const dirColor = alert.direction === "up" ? "var(--color-emerald)" : "var(--color-red)";
        return `
            <div style="display: flex; justify-content: space-between; align-items: center; background: rgba(255,255,255,0.04); padding: 4px 8px; border-radius: 4px;">
                <span>
                    <strong style="color: ${dirColor}; margin-right: 4px;">${dirIcon}</strong> Target: <strong>${alert.target.toLocaleString("ro-RO", { minimumFractionDigits: 2 })} RON</strong>
                </span>
                <button onclick="removePriceAlert(${alert.originalIndex})" style="background: none; border: none; color: var(--color-red); cursor: pointer; font-size: 14px; font-weight: bold; line-height: 1;" title="Șterge Alerta">×</button>
            </div>
        `;
    }).join("");
}
